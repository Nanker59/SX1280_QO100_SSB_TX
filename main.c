    ...
    while (true) {
        ...
        for (uint32_t n = 0; n < BLOCK_SAMPLES; n++) {
            ...
            if (duty < 1.0f) {
                ...
                float p_raw = (float)PWR_MAX_DBM + 20.0f * log10f(Aeff);

                float p_des = p_raw;
                if (p_des > (float)PWR_MAX_DBM) p_des = (float)PWR_MAX_DBM;
                ...
                if (p_high > PWR_MAX_DBM) p_high = PWR_MAX_DBM;
